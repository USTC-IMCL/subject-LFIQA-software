...
class PairWiseScoringWidget(QtWidgets.QStackedWidget):
    ...
    def __init__(self, all_lfi_info:ExpLFIInfo, exp_setting:ExpSetting, show_list):
        ...
        self.all_dist_types=all_lfi_info.GetAllDistNames()
    ...
    def SetPageShowing(self,cur_lf_info:SingleLFIInfo,init_flag=False):
        ...
        if LFIFeatures.Active_Refocusing in exp_setting.lfi_features:
            if init_flag:
                ...
                self.show_page_list[1].SetNewLFI(exp_setting,None,cur_lf_info.show_views_path,cur_lf_info.show_refocusing_views_path)
        ...
        if LFIFeatures.Passive_Refocusing in exp_setting.lfi_features:
            if init_flag:
                ...
                self.show_page_list[1].SetNewLFI(exp_setting,None,cur_lf_info.passive_refocusing_video)
...
class VideoPage(QtWidgets.QWidget):
    ...
    def SetQImage(self, image_path):
        self.img_label.setPixmap(QtGui.QPixmap.fromImage(QtGui.QImage(image_path)))
        self.resize(self.img_label.pixmap().width(), self.img_label.pixmap().height())
        self.img_label.setGeometry(0, 0, self.img_label.pixmap().width(), self.img_label.pixmap().height())
    
    def SetVideo(self, video_path, img_resolution):
        self.video_height,self.video_width=img_resolution
        self.resize(self.video_width,self.video_height)
        self.video_widget.setGeometry(0,0,self.video_width,self.video_height)
